...
function CreateMeeting() {
  const [meeting, createMeeting] = useState({
    meetingTitle: '',
    meetingDetails: '',
    meetingDate: '',
    meetingDuration: '',
  ...
  function changeHandler(e) {
    ...
    createMeeting ({...meeting, [name]: value});
  }
  const {id} = useParams()
  ...
  async function submitHandler(e){
    ...
    console.log(userId)
    try {
      const res = await axios.post(`http://localhost:8000/create/meetings`, {
        ...meeting,
        userId
      },
)
  ...
  const addMember = () => createMeeting({...meeting, 
    meetingMembers: [...meeting.meetingMembers, {name:'', email:''}]});

    const removeMember = i => createMeeting({...meeting, 
